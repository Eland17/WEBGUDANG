...
function renderTable() {
  ...
  pageItems.forEach(data => {
    ...
    // Buat td stok dengan warna sesuai kondisi stok
    let stokTd = document.createElement('td');
    stokTd.classList.add('angka-kanan');
    stokTd.textContent = sisa.toLocaleString('id-ID');

    if (sisa === 0) {
      stokTd.style.backgroundColor = '#f8d7da'; // merah muda (stok habis)
    } else if (sisa < 6) {
      stokTd.style.backgroundColor = '#fff3cd'; // kuning muda (stok rendah)
    }
  ...
  // Disable/enable tombol pagination
